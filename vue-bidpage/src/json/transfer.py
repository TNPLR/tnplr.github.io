for n in range(1,46):
    ...
    ans=old[old.index("Correct()")+11:old.index("<",old.index("Correct()"))]
    ans=ans.replace("&spades;","S")
    ans=ans.replace("&hearts;","H")
    ans=ans.replace("&diamondsuit;","D")
    ans=ans.replace("&clubs;","C")
